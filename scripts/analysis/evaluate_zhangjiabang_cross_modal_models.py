...
def _evaluate_model_set(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    feature_sets = _feature_sets(df)
    metric_rows: list[dict[str, Any]] = []
    prediction_rows: list[dict[str, Any]] = []
    for target in TARGETS:
        target_df = df[df[target].notna()].copy().reset_index(drop=True)
        if len(target_df) < 3:
            continue
...
def _json_summary(metrics: pd.DataFrame, predictions: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "notes": [
            ...
            "Success rate uses target-specific tolerances recorded in this script.",
        ],
    }


def write_outputs(output_dir: Path, metrics: pd.DataFrame, predictions: pd.DataFrame) -> dict[str, Any]:
    ...
    summary = _json_summary(metrics, predictions)
    summary["outputs"] = {
...
def main() -> None:
    ...
    supervised = _add_date_features(supervised)
    metrics, predictions = _evaluate_model_set(supervised)
    if metrics.empty:
        raise ValueError("Not enough supervised Zhangjiabang rows to evaluate models.")
    summary = write_outputs(args.output_dir, metrics, predictions)
