...
当前仓库内没有检索到可直接用于张家浜东闸站的历史连续水质时序数据。后续已补充张家浜 UAV 图片/视频和实地监测 Excel，并在 `docs/data/zhangjiabang_cross_modal_pipeline.md` 中形成跨模态处理链路；本文件保留的是此前基于近邻站点的代理时序数据说明。
...
- 当前代理时序数据本身不包含 UAV、遥感、水动力、闸门调度等字段；UAV 与实地监测已另行进入跨模态处理链路。
