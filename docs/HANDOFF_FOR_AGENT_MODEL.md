...
3. Scenario triage: compare rainfall, flow, flushing, and resuspension conditions against empirical thresholds.
...
5. Experiment orchestration: call the active scripts, check generated outputs, and report changed metrics.
...
4. Add structured response templates for `metric_summary`, `driver_diagnosis`, `threshold_check`, and `data_gap`.
5. Add future tools only after new data exists for multi-station hydrodynamics, raster/UAV boundaries, Sobol sensitivity, and counterfactual experiments.
