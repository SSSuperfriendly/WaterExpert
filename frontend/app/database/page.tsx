...
import { Search01Icon, Database01Icon } from "@hugeicons/core-free-icons";
...
export default function DatabasePage() {
  ...
  return (
    <AppShell title={t("nav.database")}>
      <PageHeading title={t("database.title")} subtitle={t("database.subtitle")} />
      ...
      {summary.loading ? (
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          <StatCard label={t("database.totalRecords")} value={formatNumber(summary.data.total_records, 0)} icon={Database01Icon} />
          <StatCard label={t("database.totalStations")} value={formatNumber(summary.data.total_stations, 0)} />
        ...
        <CardHeader className="flex-row items-center justify-between">
          ...
          {result && (
            <div className="flex gap-2">
              <Badge variant="outline">{t("database.matchedRows")}: {formatNumber(result.matched_rows, 0)}</Badge>
              <Badge variant="outline">{t("database.returnedRows")}: {formatNumber(result.returned_rows, 0)}</Badge>
            </div>
          )}
        </CardHeader>
